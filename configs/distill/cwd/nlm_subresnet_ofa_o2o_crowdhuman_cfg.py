    ...
    distiller=dict(
        ...
        components=[
            dict(
                ...
                losses=[
                    dict(
                        type='NormalizedLogitMap',
                        name='loss_nlm_cls_head',
                        tau=1,
                        loss_weight=1,
                    )
                ]),
...
work_dir = 'experiments/dis_nlm_{}_subresnet_ofa_o2o_crowdhuman_cfg_with_eval_pretrained_cls1_reg1/'.format(f)
checkpoint_config = dict(interval=10)
# log_config = dict(interval=50, hooks=[dict(type='TextLoggerHook')])
log_config = dict(interval=1,
                  hooks=[dict(type='NeptuneLoggerHook',
                              init_kwargs=dict(project='uzair789/mmdetection',
                                               name='dis_nlm_{}_subresnet_ofa_o2o_crowdhuman_cfg_with_eval_pretrained_cls1_reg1'.format(f))
                              )
                         ]
                  )
